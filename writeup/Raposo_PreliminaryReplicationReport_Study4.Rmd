...
#d0[2, 7] = .5 #replacing these 2 values from pilot A only
#d0[7, 9] = 40 #replacing these 2 values from pilot A only


d0$demos <- gsub(",Q1", "_", d0$demos) #adding separators in between demographic responses
...
d = d1[d1$exclude == "include",] #only keeping those cases should not be excluded based on criteria above
ndiff = length(unique(d0$workerid))-length(unique(d$workerid))
nincl = length(unique(d$workerid))
```
In total we excluded data from `r ndiff` participants based on the criteria above, for a total of `r nincl` participants.

**Main analyses**

&nbsp; &nbsp; &nbsp; &nbsp; To visualize the critical analysis I will plot how long participants say they will wait to begin saving by time metric condition. To visualize the secondary analysis I will plot how much participants think they should save for retirement by time metric and incremental action condition. 

&nbsp; &nbsp; &nbsp; &nbsp; Consistent with the original results, I expect that people in the fine-grained condition will want to wait less time to begin saving for retirement compared to those in the gross-grained condition. All error bars reflect +/-1 standard error.

```{r warning = FALSE, message = FALSE, echo = FALSE, fig.width=4, fig.height=3.5}
delaytimemean = d %>% 
  group_by(timeMetric) %>%
  summarise(mean=mean(delaytimeyrs, na.rm = T), sd=sd(delaytimeyrs, na.rm = T), n = sum(!is.na(delaytimeyrs)), sem = sem(delaytimeyrs))
#delaytimemean
...
ggplot(savingtable, aes(x = timemetric, y = mean, fill = action)) + 
  ...
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
            ...
            width=.2,                    # Width of the error bars
            position=position_dodge(.9)) +
  coord_cartesian(ylim=c(11,13.5)) + 
  xlab("Time metric condition") +
  ylab("Log total retirement savings ($)") +
  ggtitle("Retirement Savings (log) by \n Condition - Original") +
  theme_linedraw(base_size = 11)
...
##### Critical analysis: Effect of time metric and incremental action on when to save

&nbsp; &nbsp; &nbsp; &nbsp; Consistent with the graphs above, there were two main analyses in the original paper. The primary analysis of interest was whether there were differences between time metric condition on WHEN to begin saving for retirement. The secondary analysis of interest was whether there were differences between time metric conditions or incremental action conditions on how much to save. In both cases, the authors ran a regression model with no demographic controls, and a model with demographic controls (age, education, income). Only income significantly predicted amount to save, but controlling for demographics did not change the interpretation of either outcome.

&nbsp; &nbsp; &nbsp; &nbsp; Consistent with the original paper, I expect a main effect of time metric such that people in the fine-grained condition would begin to save sooner than those in the gross-grained condition. I expect a similar effect regardless of whether I control for demographics.

```{r warning = FALSE, message = FALSE, echo = FALSE}
#controlling for demographics 
rs.lmdelaytot.cov = lm(delaytimeyrs ~ age + income + edu + timeMetric * incrAction, d)
anova(rs.lmdelaytot.cov)
...
##### Secondary analysis: Effect of time metric and incremental action on how much to save (log transformed)
...
```{r warning = FALSE, message = FALSE, echo = FALSE}
#controlling for demographics
rs.lmlogsavingstot.cov = lm(logsavingtot ~ timeMetric * incrAction + age + edu + income, d)
anova(rs.lmlogsavingstot.cov) 
...
```{r demos, message = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
