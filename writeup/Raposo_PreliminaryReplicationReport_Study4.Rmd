...
&nbsp; &nbsp; &nbsp; &nbsp; I began by visualizing my data by using histograms and boxplots (to see ranges and detect outliers). Next, I will use bar plots and line graphs to better visualize my data by condition. 
...
```
For the critical analysis the main effect of time metric is: F(`r dfnum1`, `r dfdenom1`) = `r round(Fstat1, 3)`, p = `r round(pval1, 4)`. When not controlling for demographics, the main effect of time metric is:  F(`r dfnum2`, `r dfdenom2`) = `r round(Fstat2, 3)`, p = `r round(pval2, 4)`.
...
```
For the secondary analysis (controlling for demographics) the main effect of incremental action is: F(`r dfnum3b`, `r dfdenom3b`) = `r round(Fstat3b, 3)`, p = `r round(pval3b, 4)` and the main effect of time metric is: F(`r dfnum3a`, `r dfdenom3a`) = `r round(Fstat3a, 3)`, p = `r round(pval3a, 4)`.
