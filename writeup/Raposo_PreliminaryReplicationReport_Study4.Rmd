...
&nbsp; &nbsp; &nbsp; &nbsp; The reported effect size for the original statistically significant main effect with 122 participants was d = 0.51. Post-hoc power calculation indicated that power for the original study was 0.79. Assuming an effect size of d = 0.51, I would require N = 123 to achieve 80% power, N = 164 to achieve 90% power, and N = 202 to achieve 95% power. I will aim to achieve 80% power, so will require a sample size of N = 123. This sample size gives me confidence that I should find an effect if it is present and meets the cost requirements of the course.
...
&nbsp; &nbsp; &nbsp; &nbsp; I will begin by visualizing my data by using histograms and boxplots (to see ranges and detect outliers). Next, I will use bar plots and line graphs to better visualize my data by condition. 

&nbsp; &nbsp; &nbsp; &nbsp; To answer the main research questions, I will run 2 regression models to examine the interactive effects of time metric and incremental action on: a) when to begin saving, b) savings amount natural log-transformed (base e). I will run both models with and without demographic covariates (age, education, income) consistent with the original paper. To further explore the data, I will also run a regression model on total savings amount. 

&nbsp; &nbsp; &nbsp; &nbsp; Because my sample means may differ from those of the original I will compare the group means I obtain from my sample on the 2 dependent variables to those obtained in the original study. To further explore the data I also will examine the relationship between when to save and savings amount in my sample. I will examine whether there are relationships between the two dependent variables and demographic covariates. In the original study, only income was found to be positively associated with amount to save. 
...
d0[2, 7] = .5 #replacing these 2 values from pilot only
d0[7, 9] = 40 #replacing these 2 values from pilot only


d0$demos <- gsub(",Q1", "_", d0$demos)
d0$demos <- gsub(",Q2", "_", d0$demos)
d0$demos <- gsub(",Q3", "_", d0$demos)
d0$comments <- gsub("Q0", "", d0$comments) #adding separators in between demographic responses
...
#str(d.demos$educat)
#d.demos$gendercat = factor(d.demos$gender, labels = c("Male", "Female", "Other", "Decline to state"))
#d.demos$educat = factor(d.demos$educat, labels = c("high school diploma or less", "some college or technical school", "college degree", "some post-graduate", "post-graduate degree (e.g., Masters, Ph.D., J.D.)", "Decline to state"))
#d.demos$racecat = factor(d.demos$race, labels = c("White non-Hispanic", "White Hispanic", "Black or African American", "American Indian or Alaska Native", "Asian", "Native Hawaiian or Pacific Islander", "Decline to state"))
#d.demos$incomecat = factor(d.demos$income, labels = c("<$20,000", "$20-$29,999", "$30-$39,999", "$40-$49,999", "$50-$59,999", "$60-$69,999", "$70-$79,999", "$80-$89,999", "$90-$99,999", "$100,000+", "Decline to state"))
...
Participants were `r length(unique(d.raw$workerid))` MTurk workers.

```{r warning = FALSE, message = FALSE, echo = FALSE}

#Reading in tidy file
setwd("~/Desktop/sraposoProject/ignore/data")
d0 = read.csv("LewisReplication_PilotData_Tidy.csv")
...
#table(d0$edu)
#d0$educat = factor(d0$edu, labels = c("high school diploma or less", "some college or technical school", "college degree", "some post-graduate", "post-graduate degree (e.g., Masters, Ph.D., J.D.)", "Decline to state"))
#table(d0$educat)

#table(d0$income)
#d0$incomecat = factor(d0$income, labels = c("<$20,000", "$20-$29,999", "$30-$39,999", "$40-$49,999", "$50-$59,999", "$60-$69,999", "$70-$79,999", "$80-$89,999", "$90-$99,999", "$100,000+", "Decline to state"))
#table(d0$incomecat)

#d0$gendercat = factor(d0$gender, labels = c("male", "female", "other", "decline to state"))
#table(d0$gendercat)

#d0$racecat = factor(d0$race, labels = c("White", "Hispanic", "Black", "AmerInd/AlaskaNative", "Asian", "PacifIslander", "decline to state"))
#table(d0$racecat)
...
d0$savingdiv <- ifelse((d0$timeMetric == "fine") & (d0$incrAction) == "incremental", 10950, 
...
d0$savingtot = d0$savings * d0$savingdiv #Calculating overall retirement savings
...
&nbsp; &nbsp; &nbsp; &nbsp; First it is important to exclude participants based on my predetermined exclusion criteria. I will exclude participants who did not respond to the dependent variables, who reported they would wait longer than 10,950 days to begin saving in the fine-grained condition, or longer than 30 years in the gross-grained condition, and those who gave responses that fell more than three standard deviations above the mean on the dependent variables.
...
d0$delayexclude <- ifelse(d0$delaytimeyrs > (mean(d0$delaytimeyrs, na.rm = T) + 3*sd(d0$delaytimeyrs, na.rm = T)), "exclude", "include") 
tdelayexcl = table(d0$delayexclude)

d0$delayrawexclude1 <- ifelse((d0$timeMetric == "fine" & d0$delaytime > 10950), "exclude", "include") 
tdelayexclraw1 = table(d0$delayrawexclude1)

d0$delayrawexclude2 <- ifelse((d0$timeMetric == "gross" & d0$delaytime > 30), "exclude", "include") 
tdelayexclraw2 = table(d0$delayrawexclude2)

d0$exclude <- ifelse((d0$delayexclude == "exclude" | d0$delayrawexclude1 == "exclude" | d0$delayrawexclude2 == "exclude"), "exclude", "include") 
#d0$exclude = factor(d0$exclude, labels = c("exclude", "include"))
tableexcl = table(d0$exclude)

d = d0[d0$exclude == "include",] #only keeping those cases that I decided to include

```
In total we excluded data from `r tableexcl[1]` participants. We excluded `r tdelayexclraw1[1]` participants who would wait longer than 10,950 days in the fine-grained condition and `r tdelayexclraw2[1]` participants who would wait longer than 30 years to begin saving in the gross-grained condition. `r tdelayexcl[1]` participants gave responses > 3SD from the mean on when they would begin to save. 

**Main analyses**

&nbsp; &nbsp; &nbsp; &nbsp; To begin the analyses I will plot how long people say they will wait to begin saving by time metric condition, and how much they think they should save for retirement by time metric and incremental action condition. 

&nbsp; &nbsp; &nbsp; &nbsp; Consistent with the original results, I expect that people in the fine-grained condition will want to wait less time to begin saving for retirement compared to those in the gross-grained condition.

```{r warning = FALSE, message = FALSE, echo = FALSE, fig.width=4.5, fig.height=3.5}
delaytimemean = d %>% 
  group_by(timeMetric) %>%
  summarise(mean=mean(delaytimeyrs, na.rm = T), sd=sd(delaytimeyrs, na.rm = T), n = sum(!is.na(delaytimeyrs)), sem = sem(delaytimeyrs))
#delaytimemean
...
&nbsp; &nbsp; &nbsp; &nbsp; Now, consistent with the original paper, I will plot how much people report they should save for retirement (natural log transformed) by time metric and incremental action conditions. 

```{r warning = FALSE, message = FALSE, echo = FALSE, fig.width=5.5, fig.height=3.5}
logsavingtotmean = d %>% 
...
#  ylim(c(0,13)) + #to match original study
  xlab("Time metric condition") + 
...
&nbsp; &nbsp; &nbsp; &nbsp; Consistent with the graphs above, there were two main analyses in the original paper. The first was whether there were differences between time metric condition on WHEN to begin saving for retirement. The second was whether there were differences between time metric conditions or incremental action conditions on how much to save. In both cases, the authors ran a regression model with no demographic controls, and a model with demographic controls (age, education, income). Only income significantly predicted amount to save, but controlling for demographics did not change the interpretation of either outcome.

##### Effect of time metric and incremental action on when to save

&nbsp; &nbsp; &nbsp; &nbsp; Consistent with the original paper, I expect a main effect of time metric such that people in the fine-grained condition would begin to save sooner than those in the gross-grained condition.
...
rs.lmdelaytot.cov = lm(delaytimeyrs ~ timeMetric * incrAction + age + income + edu, d)

```



##### Effect of time metric and incremental action on how much to save (log transformed)

&nbsp; &nbsp; &nbsp; &nbsp; Consistent with the original paper, I expect no effect of time metric or incremental action on the natural log-transformed values of how much to save overall for retirement.

```{r warning = FALSE, message = FALSE, echo = FALSE, eval = FALSE}
#no demographic controls
rs.lmlogsavingstot = lm(logsavingtot ~ timeMetric * incrAction, d); summary(rs.lmlogsavingstot) 
anova(rs.lmlogsavingstot)

#controlling for demographics
rs.lmlogsavingstot.cov = lm(logsavingtot ~ timeMetric * incrAction + age + edu + income, d); summary(rs.lmlogsavingstot.cov) 
```


#### Comparing means from our sample to those in original paper

&nbsp; &nbsp; &nbsp; &nbsp; Because our sample may different somewhat from the sample in the original paper, it is important to compare sample means. I will run t-tests to compare means of when to start saving in each condition, and how much to save (log transformed) to those in the original paper.

**Means of when to start saving**

In the original paper, participants in the fine-grained condition reported wanting to wait an average of 0.45 years.
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = FALSE}
t.test(d$delaytimeyrs[d$timeMetric == "fine"], mu = .45) #comparing to mean(fine) in original study
```

In the original paper, participants in the gross-grained condition reported wanting to wait an average of 1.70 years.
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = FALSE}
t.test(d$delaytimeyrs[d$timeMetric == "gross"], mu = 1.70) #comparing to mean(gross) in original study
```

**Means of how much to save**

In the original paper, participants in the fine-grained, incremental condition reported wanting to save $224,134 (12.32 in log).

```{r warning = FALSE, message = FALSE, echo = FALSE, eval = FALSE}
t.test(d$logsavingtot[d$timeMetric == "fine" & d$incrAction == "incremental"], mu = 12.32)
t.test(d$savingtot[d$timeMetric == "fine" & d$incrAction == "incremental"], mu = 224134) 
```

In the original paper, participants in the fine-grained, overall condition reported wanting to save $215,345 (12.28 in log).
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = FALSE}
t.test(d$logsavingtot[d$timeMetric == "fine" & d$incrAction == "overall"], mu = 12.28) 
t.test(d$savingtot[d$timeMetric == "fine" & d$incrAction == "overall"], mu = 215345) 
```

In the original paper, participants in the gross-grained, incremental condition reported wanting to save $240,385 (12.39 in log).
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = FALSE}
t.test(d$logsavingtot[d$timeMetric == "gross" & d$incrAction == "incremental"], mu = 12.39) 
t.test(d$savingtot[d$timeMetric == "gross" & d$incrAction == "incremental"], mu = 240385) 
```

In the original paper, participants in the gross-grained, overall condition reported wanting to save $451,350 (13.02 in log).
````{r warning = FALSE, message = FALSE, echo = FALSE, eval = FALSE}
t.test(d$logsavingtot[d$timeMetric == "gross" & d$incrAction == "overall"], mu = 13.02) 
t.test(d$logsavingtot[d$timeMetric == "gross" & d$incrAction == "overall"], mu = 451350) 
```
...
We should also plot when to start saving (in years) across all 4 conditions, rather than only across the time metric conditions (which is consistent with the original paper). This includes the incremental action condition. We would expect that there would be no effect of incremental action condition because this condition was not assigned/did not come into play until after the when to save question was asked.
...
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = FALSE, fig.width=5, fig.height=3}
